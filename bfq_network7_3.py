...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, class_aux):
        self.inplanes = 64
        ...
        self.class_aux = class_aux
        class_aux_pascal = self.class_aux + 15
        self.Fc = nn.Linear(256, class_aux_pascal, bias=False)
    ...
    def forward(self, query_rgb,support_rgb,support_mask,history_mask,history_mask_bk=None, subcls=None):
        ...
        fore_prototype = F.avg_pool2d(input=z,
                         kernel_size=support_rgb.shape[-2:]) * h * w / area

        if self.training:
            feat_input = fore_prototype.squeeze(-1).squeeze(-1) #torch.Size([batch size,256])
            #add Fc loss for base classes
            Fc_calculate = self.Fc(feat_input)[:,:15] #torch.Size([batch size,15])
            loss_class_fore = self.criterion(Fc_calculate, subcls)

            self_matrix = self.Fc.weight @ self.Fc.weight.T # 15+class_aux * 15+class_aux
            regula_lable = torch.arange(self_matrix.size(0)).cuda()
            loss_regula = self.loss_CrossEntropy(self_matrix, regula_lable)

            query_prototypes = F.avg_pool2d(input=query_rgb, kernel_size=query_rgb.shape[-2:])
            for i in range(query_prototypes.size(0)):
                bk_prototype_one_list = []
                query_feat_input = torch.reshape(query_prototypes[i,:,:,:], (256, 1))
                query_feat_score = self.Fc(query_feat_input.t())
                #obtain bk prototypes from multiply query_feat_score and weight, delete current class prototypes
                class_all_tmp = torch.zeros(15+self.class_aux).cuda()
                class_all_tmp[subcls[i]] = 1
                query_feat_score_tmp = query_feat_score[:,class_all_tmp == 0]
                weight_tmp = self.Fc.weight[class_all_tmp == 0]
                bk_prototype_tmp = query_feat_score_tmp.mm(weight_tmp)
                #import ipdb
                #ipdb.set_trace()
                bk_prototype_onebatch = torch.reshape(bk_prototype_tmp.t(), (256, 1, 1)).unsqueeze(0)
                if i == 0:
                    bk_prototype = bk_prototype_onebatch
                else:
                    bk_prototype = torch.cat((bk_prototype, bk_prototype_onebatch), 0)
            
        z = fore_prototype.expand(-1, -1, feature_size[0], feature_size[1])  # tile for cat
        ...
        if self.training:
            history_mask_bk=F.interpolate(history_mask_bk,feature_size,mode='bilinear',align_corners=True)
            z_bk = bk_prototype.expand(-1, -1, feature_size[0], feature_size[1])  # tile for cat
            out=torch.cat([query_rgb,z_bk],dim=1)
            out = self.layer55(out)
            #value_tmp = torch.max(history_mask) + torch.min(history_mask)
            #history_mask_bk = value_tmp - history_mask
            out_plus_history=torch.cat([out,history_mask_bk],dim=1)
        ...
        if self.training:
            return out, out_bk, loss_class_fore, loss_regula
        else:
            return out
...
def Res_Deeplab(num_classes=2, class_aux=15):
    model = ResNet(Bottleneck,[3, 4, 6, 3], num_classes, class_aux)
    return model
