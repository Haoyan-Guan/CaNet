...
class Dataset(object):
    ...
    def __init__(self, data_dir, fold, input_size=[321, 321] , normalize_mean=[0, 0, 0],
        ...
        self.history_mask_bk_list = [None] * self.__len__()
    ...
    def __getitem__(self, index):
        ...
        #add bk
        if self.history_mask_bk_list[index] is None:
            history_mask_bk=torch.zeros(2,41,41).fill_(0.0)
        else:
            if random.random()>self.prob:
                history_mask_bk=self.history_mask_bk_list[index]
            else:
                history_mask_bk = torch.zeros(2, 41, 41).fill_(0.0)

        return query_rgb, query_mask, support_rgb, support_mask,history_mask,history_mask_bk,sample_class,index
